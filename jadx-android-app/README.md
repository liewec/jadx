...
v1.3.1 修复了 MCP 后台使用期间服务日志丢失的问题（日志改为环形缓冲持久记录，返回页面可完整回看）；服务地址区分**本地地址**（AI 与本应用同一台手机）与**局域网地址**（AI 在其他设备）并分别一键复制；使用教程合并为一段可整体选择复制的文本。

v1.3.2 修复了 MCP 服务页底部内容被底部导航栏遮挡的问题。

v1.3.3 修复了 MCP `load_file` 按路径读取 Download 等目录报 `EACCES` 的权限问题：MCP 服务页新增文件访问权限一键引导（Android 11+ 跳转"所有文件访问"设置）。
...
- **MCP 服务界面**（v1.3）：一键启动/停止 MCP 服务端，本地/局域网双地址一键复制，实时日志（后台使用后可完整回看），内置可整体选择复制的使用教程
...
- **双地址一键复制**（v1.3.1）：AI 软件与本应用同一台手机 → 复制本地地址 `http://127.0.0.1:33333/mcp`；AI 软件在电脑/其他设备 → 复制局域网地址 `http://<手机IP>:33333/mcp`
- **前台服务保活**：切到 AI 应用后服务不中断
- **日志持久记录**（v1.3.1）：日志写入环形缓冲并带时间戳，后台使用期间产生的请求日志不丢失，返回服务页可完整回看 AI 的调用记录
...
2. **授予文件访问权限**（v1.3.3）：若页面顶部出现 "⚠ 未授予'所有文件访问'权限" 提示，点击 **授权** 按钮——Android 11+ 会跳转系统设置页，开启"允许管理所有文件"后返回；Android 10 及以下弹出存储权限对话框确认。未授权时 AI 调用 `load_file` / `list_dir` 读取 Download 等目录会报 `EACCES`
3. 确认端口（默认 `33333`，可自定义 1024–65535）
4. 点击 **启动服务**，首次会请求通知权限（用于前台服务常驻）
5. 状态变为 "● MCP 服务运行中"，页面显示两个服务地址（本地地址 + 局域网地址）

> 启动后可切换到其他应用，前台服务保持运行；通知栏常驻 "Dex 编辑器 MCP 服务运行中"。

#### 第二步：选择并复制服务地址

服务页区分两种地址，按 AI 软件所在位置选择（v1.3.1）：

| 场景 | 使用地址 | 示例 |
|------|----------|------|
| 带 MCP 的 AI 软件与本应用在**同一台手机** | 本地地址 | `http://127.0.0.1:33333/mcp` |
| AI 软件在**电脑 / 其他手机**（需与手机同一 Wi-Fi） | 局域网地址 | `http://192.168.1.5:33333/mcp` |

点击对应地址行的 **复制** 按钮即可一键复制（点击地址本身也可复制）。

> 手机 IP 查看方式：系统设置 → WLAN → 点击当前网络 → 查看 IP 地址。MCP 服务页显示的局域网地址即当前 IP。

#### 第三步：在 AI 软件中添加服务器

1. 在支持 MCP 的 AI 软件（Claude、ChatMCP、Cursor 等）的 **MCP 服务器 / MCP 设置** 中新增服务器
2. 类型选择 **Streamable HTTP**（或 HTTP），粘贴上一步复制的地址
3. 老版本客户端仅支持 **SSE** 类型：地址结尾把 `/mcp` 换成 `/sse`

电脑端 Claude Desktop / Cursor 等也可在 MCP 配置文件中添加：
...
（把 `192.168.1.5` 换成你手机的局域网 IP；同一台手机上的 AI 软件则用 `http://127.0.0.1:33333/mcp`）

连接成功后，AI 会发现 15 个工具（get_status / load_file / decompile_class …）。
...
返回本应用的 MCP 服务页，可在 **服务日志** 回看 AI 的调用记录（带时间戳；即使使用期间本应用在后台，日志也不会丢失）。
...
#### Bug 修复
- **修复 MCP `load_file` 读取 APK 报 `EACCES (Permission denied)`**：AI 通过路径加载 `/storage/emulated/0/Download/...` 下的文件时因分区存储权限不足被拒。
  - MCP 服务页新增**文件访问权限引导**：未授权时显示醒目提示行，点击"授权"按钮，Android 11+ 跳转系统"所有文件访问"设置页，Android 10 及以下运行时申请存储权限；从设置返回后自动刷新状态
  - `load_file` / `list_dir` 失败时的错误信息附上明确的授权指引（AI 可直接转述给用户），并区分"文件不存在"与"权限不足"两种情况
  - Manifest 补充 `requestLegacyExternalStorage`（Android 10 兼容）
  - 使用教程第一步补充权限授予说明

#### 其他
- 版本号：versionCode=7，versionName=1.3.3；关于对话框更新至 v1.3.3

### v1.3.2（2026-08-16）

#### Bug 修复
- **修复 MCP 服务页底部内容被导航栏遮挡**：使用教程末尾的"安全提示"（服务无鉴权警告）被底部导航栏盖住不可见。现页面 ScrollView 增加底部内边距（72dp，`clipToPadding=false`），滚动到底时最后内容完整显示在导航栏上方

#### 其他
- 版本号：versionCode=6，versionName=1.3.2；关于对话框更新至 v1.3.2

### v1.3.1（2026-08-16）

#### Bug 修复
- **修复 MCP 服务日志丢失**：此前切到 AI 软件期间（本应用后台、页面暂停）产生的请求日志会被丢弃，返回服务页后看不到任何日志。现日志改为服务端静态环形缓冲持久记录（带 `HH:mm:ss` 时间戳，约 16KB 上限），返回页面时从快照完整回看；服务的启动成功/失败/停止也写入日志

#### 新功能
- **服务地址区分与一键复制**：
  - **本地地址**（`http://127.0.0.1:<端口>/mcp`）：带 MCP 的 AI 软件与本应用在同一台手机时使用
  - **局域网地址**（`http://<手机IP>:<端口>/mcp`）：AI 软件在电脑/其他设备时使用（需同一 Wi-Fi）
  - 每个地址独立"复制"按钮（点击地址本身也可复制），标签注明适用场景
- **教程整体可复制**：使用教程合并为单个可选择文本（`textIsSelectable`），长按即可全选复制；教程内容同步更新为"选择并复制地址 → 添加服务器"新流程

#### 其他
- 版本号：versionCode=5，versionName=1.3.1；关于对话框更新至 v1.3.1
